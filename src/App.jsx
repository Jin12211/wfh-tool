    ...
    for (let m = 0; m < 3; m++) {
      ...
      for (let i = 0; i < days; i++)
        r.push({ id:`${emp["Employee #"]}-${m}-${i}`, employeeId:emp["Employee #"], team:emp.Teams,
          ...
          half: null, status:["approved","approved","pending","rejected"][Math.floor(Math.random()*4)], reason:"WFH request" });
    }
  });
  return r;
};
...
// ── UI primitives ─────────────────────────────────────────────────────────────
const Tag = ({ status }) => {
  const cfg = { approved:{bg:"#1a2e1a",color:"#4ade80"}, pending:{bg:"#2e2510",color:C.yellow}, rejected:{bg:"#2e1410",color:C.red} }[status] || {bg:C.g08,color:C.g03};
...
const Btn = ({ children, onClick, variant="primary", small=false, full=false }) => {
  const s = { primary:{background:C.white,color:C.black,border:"none"}, ghost:{background:"transparent",color:C.white,border:`1px solid ${C.g06}`}, danger:{background:"transparent",color:C.red,border:`1px solid ${C.red}`}, success:{background:"transparent",color:"#4ade80",border:"1px solid #4ade80"}, yellow:{background:C.yellow,color:C.black,border:"none"} }[variant];
  return <button onClick={onClick} style={{...s,fontFamily:"inherit",fontWeight:600,fontSize:small?11:13,padding:small?"4px 12px":"9px 20px",cursor:"pointer",borderRadius:2,letterSpacing:"0.03em",width:full?"100%":"auto",transition:"opacity .15s"}} onMouseEnter={e=>e.currentTarget.style.opacity=".75"} onMouseLeave={e=>e.currentTarget.style.opacity="1"}>{children}</button>;
};
...
const Card = ({ title, children, accent }) => (
  <div style={{background:C.g09,border:`1px solid ${C.g08}`,borderRadius:2,borderTop:accent?`2px solid ${accent}`:undefined,padding:20,marginBottom:12}}>
    {title && <div style={{color:C.g03,fontSize:11,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:16}}>{title}</div>}
    {children}
  </div>
);
const THead = ({ cols }) => (
  <thead><tr style={{borderBottom:`1px solid ${C.g08}`}}>{cols.map(c=><th key={c} style={{textAlign:"left",color:C.g04,fontSize:11,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase",padding:"6px 16px 6px 0"}}>{c}</th>)}</tr></thead>
);
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2}}>
        ...
        {Array.from({length:daysInMonth},(_,i)=>i+1).map(d=>{
          ...
          const weekend = isWeekend(d); const past = isPast(d);
          const disabled = weekend || past;
          const selected2 = !!entry;
          return (
            <div key={d} onClick={()=>toggleDay(d)} style={{
              textAlign:"center",padding:"8px 4px",borderRadius:2,cursor:disabled?"not-allowed":"pointer",
              background: selected2 ? C.yellow : "transparent",
              color: disabled ? C.g07 : selected2 ? C.black : C.white,
              fontWeight: selected2 ? 700 : 400, fontSize:13,
              border: `1px solid ${selected2 ? C.yellow : "transparent"}`,
              transition:"all .1s"
            }}>{d}</div>
          );
        })}
      ...
      {/* Half day options */}
      {selected.filter(s => s.date.startsWith(`${view.y}-${String(view.m+1).padStart(2,"0")}`)).length > 0 && (
        <div style={{marginTop:16}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            {selected.filter(s=>s.date.startsWith(`${view.y}-${String(view.m+1).padStart(2,"0")}`)).map(s=>(
              <div key={s.date} style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:C.g08,padding:"8px 12px",borderRadius:2}}>
                ...
                <div style={{display:"flex",gap:6}}>
                  <button onClick={()=>setHalf(s.date,"AM")} style={{background:s.half==="AM"?C.yellow:"transparent",color:s.half==="AM"?C.black:C.g03,border:`1px solid ${s.half==="AM"?C.yellow:C.g06}`,fontFamily:"inherit",fontWeight:700,fontSize:11,padding:"3px 10px",borderRadius:2,cursor:"pointer",letterSpacing:"0.05em"}}>½ AM</button>
                  <button onClick={()=>setHalf(s.date,"PM")} style={{background:s.half==="PM"?C.yellow:"transparent",color:s.half==="PM"?C.black:C.g03,border:`1px solid ${s.half==="PM"?C.yellow:C.g06}`,fontFamily:"inherit",fontWeight:700,fontSize:11,padding:"3px 10px",borderRadius:2,cursor:"pointer",letterSpacing:"0.05em"}}>½ PM</button>
                  <button onClick={()=>setHalf(s.date,null)} style={{background:"transparent",color:s.half===null?C.white:C.g05,border:`1px solid ${s.half===null?C.white:C.g07}`,fontFamily:"inherit",fontWeight:700,fontSize:11,padding:"3px 10px",borderRadius:2,cursor:"pointer"}}>Full</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {selected.length > 0 && (
        <div style={{marginTop:10,color:C.g04,fontSize:12}}>
          {selected.length} day(s) selected &nbsp;·&nbsp; {selected.reduce((a,s)=>a+(s.half?0.5:1),0)} total days
        </div>
      )}
    </div>
  );
...
// ── Login ─────────────────────────────────────────────────────────────────────
const LoginScreen = ({ employees, onLogin, invites }) => {
  const [email, setEmail] = useState(""); const [pw, setPw] = useState(""); const [err, setErr] = useState("");
  const go = () => {
    const emp = employees.find(e => e["Work Email"].toLowerCase() === email.toLowerCase().trim());
    if (!emp) { setErr("No account found for this email."); return; }
  ...
  return (
    <div style={{minHeight:"100vh",background:C.black,display:"flex",alignItems:"center",justifyContent:"center",padding:24,fontFamily:"'Inter',sans-serif"}}>
      <div style={{width:"100%",maxWidth:400}}>
        ...
        <div style={{marginTop:20,color:C.g06,fontSize:11,letterSpacing:"0.03em"}}>
          Default password for all accounts: <span style={{color:C.g04}}>1234</span>
        </div>
      </div>
    </div>
  );
...
const NewRequestModal = ({ onSubmit, onClose }) => {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.8)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,padding:24,overflowY:"auto"}}>
      <div style={{background:C.g09,border:`1px solid ${C.g07}`,borderTop:`2px solid ${C.yellow}`,borderRadius:2,padding:28,width:"100%",maxWidth:480}}>
        <div style={{color:C.white,fontWeight:700,fontSize:18,letterSpacing:"-0.02em",marginBottom:20}}>New WFH Request</div>
        <Field label="Select Dates (click to select, click again to deselect)">
  ...
  return (
    <div>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
        ...
        {pending.length>0&&<span style={{background:C.red,color:C.white,borderRadius:2,fontSize:11,fontWeight:700,padding:"2px 8px"}}>{pending.length}</span>}
      </div>
      {pending.length===0
        ? <div style={{color:C.g05,fontSize:14,padding:"40px 0",textAlign:"center"}}>No pending requests.</div>
        : <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {pending.map(r=>{
              const emp=getEmp(r.employeeId);
              return (
                <div key={r.id} style={{background:C.g09,border:`1px solid ${C.g08}`,borderRadius:2,padding:"16px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:16}}>
                  <div>
                    <div style={{color:C.white,fontWeight:600,fontSize:14}}>{emp?.["Last name, First name"]}</div>
                    <div style={{color:C.g04,fontSize:12,marginTop:2}}>{emp?.["Job Title"]} · {r.date}{r.half?` (½${r.half})`:""} · {r.half?0.5:1} day</div>
...
const TeamDash = ({ user, employees, records }) => {
  ...
  const monthData = Array.from({length:3},(_,i)=>{ const d=new Date(now.getFullYear(),now.getMonth()-(2-i),1); return {l:MONTHS[d.getMonth()].slice(0,3),v:teamRecs.filter(r=>r.date.startsWith(d.toISOString().slice(0,7))).length}; });
  const memberData = myTeamIds.map(id=>{ const emp=employees.find(e=>e["Employee #"]===id); const name=emp?.["Last name, First name"]??id; const parts=name.split(","); const full=parts.length>1?`${parts[1].trim()} ${parts[0].trim()}`:name; return {l:full,v:teamRecs.filter(r=>r.employeeId===id).length}; });
  return (
    <div>
      <div style={{color:C.white,fontWeight:700,fontSize:20,letterSpacing:"-0.03em",marginBottom:20}}>Team Dashboard</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
        <Card title="WFH Days by Month" accent={C.yellow}><Bar data={monthData}/></Card>
        <Card title="WFH Days by Member" accent={C.g06}><Bar data={memberData} accent={C.g04}/></Card>
      </div>
      <Card title="Team Members">
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          <THead cols={["Name","Title","WFH Days"]}/>
          <tbody>{myTeamIds.map(id=>{ const emp=employees.find(e=>e["Employee #"]===id); return <tr key={id} style={{borderBottom:`1px solid ${C.g09}`}}><td style={{color:C.white,fontSize:13,padding:"10px 16px 10px 0"}}>{emp?.["Last name, First name"]}</td><td style={{color:C.g04,fontSize:13,padding:"10px 16px 10px 0"}}>{emp?.["Job Title"]}</td><td style={{color:C.yellow,fontSize:13,fontWeight:700,padding:"10px 0"}}>{teamRecs.filter(r=>r.employeeId===id).length}</td></tr>; })}</tbody>
        </table>
      </Card>
    </div>
  );
...
const OrgDash = ({ employees, records }) => {
  ...
  const monthData = Array.from({length:3},(_,i)=>{ const d=new Date(now.getFullYear(),now.getMonth()-(2-i),1); return {l:MONTHS[d.getMonth()].slice(0,3),v:approved.filter(r=>r.date.startsWith(d.toISOString().slice(0,7))).length}; });
  return (
    <div>
      ...
      <Card title="All Employees">
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          ...
          <tbody>{employees.map(emp=><tr key={emp["Employee #"]} style={{borderBottom:`1px solid ${C.g09}`}}><td style={{color:C.white,fontSize:13,padding:"10px 16px 10px 0"}}>{emp["Last name, First name"]}</td><td style={{color:C.g04,fontSize:13,padding:"10px 16px 10px 0"}}>{emp.Teams}</td><td style={{color:C.g04,fontSize:13,padding:"10px 16px 10px 0"}}>{emp["Job Title"]}</td><td style={{color:C.yellow,fontSize:13,fontWeight:700,padding:"10px 0"}}>{approved.filter(r=>r.employeeId===emp["Employee #"]).length}</td></tr>)}</tbody>
        </table>
      </Card>
    </div>
  );
...
const AdminPanel = ({ employees, setEmployees, records, setRecords, invites, setInvites }) => {
  ...
  const [editReporting, setEditReporting] = useState("");
  ...
  const handleFile = e => {
    ...
    r.onload = ev => {
      try {
        ...
        if (parsed.length) {
          const withPw = parsed.map(p => ({...p, Password: p.Password || "1234"}));
          setEmployees(withPw); setRecords(genRecords(withPw));
          alert(`✅ Imported ${parsed.length} employees.`);
        }
  ...
  const sendInvite = () => {
    ...
    setInvites(prev => [...prev, { email: inviteEmail, sentAt: new Date().toISOString().slice(0,10) }]);
    setInviteMsg(`✅ Invite sent to ${inviteEmail} (simulated — real email needs backend)`);
    setInviteEmail("");
  };

  const saveReporting = (id) => {
    setEmployees(prev => prev.map(e => e["Employee #"] === id ? {...e, "Reporting to": editReporting} : e));
    setEditingId(null);
  };
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:0,marginBottom:20,borderBottom:`1px solid ${C.g08}`}}>
        {SUBTABS.map(t=>(
          <button key={t.k} onClick={()=>setSubTab(t.k)} style={{background:"none",border:"none",borderBottom:subTab===t.k?`2px solid ${C.yellow}`:"2px solid transparent",color:subTab===t.k?C.white:C.g04,fontFamily:"inherit",fontSize:13,fontWeight:subTab===t.k?600:400,padding:"10px 16px",cursor:"pointer"}}>{t.l}</button>
        ))}
      </div>
      ...
      {subTab==="employees" && (
        <div>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
            <div style={{color:C.g03,fontSize:12}}>Upload a CSV file to import employees. Required columns: Last name, First name · Employee # · Work Email · Teams · Job Title · Reporting to</div>
            <label style={{background:C.white,color:C.black,fontSize:13,fontWeight:600,padding:"8px 20px",borderRadius:2,cursor:"pointer",whiteSpace:"nowrap",marginLeft:16}}>
              Import CSV <input ref={fileRef} type="file" accept=".csv" style={{display:"none"}} onChange={handleFile}/>
            </label>
          </div>
          <Card>
            <div style={{overflowX:"auto"}}>
              <table style={{width:"100%",borderCollapse:"collapse"}}>
                <THead cols={["#","Name","Email","Team","Title","Reports To"]}/>
                <tbody>{employees.map(emp=><tr key={emp["Employee #"]} style={{borderBottom:`1px solid ${C.g09}`}}><td style={{color:C.g04,fontSize:12,padding:"10px 16px 10px 0"}}>{emp["Employee #"]}</td><td style={{color:C.white,fontSize:12,padding:"10px 16px 10px 0"}}>{emp["Last name, First name"]}</td><td style={{color:C.g04,fontSize:12,padding:"10px 16px 10px 0"}}>{emp["Work Email"]}</td><td style={{color:C.g04,fontSize:12,padding:"10px 16px 10px 0"}}>{emp.Teams}</td><td style={{color:C.g04,fontSize:12,padding:"10px 16px 10px 0"}}>{emp["Job Title"]}</td><td style={{color:C.g04,fontSize:12,padding:"10px 0"}}>{emp["Reporting to"]}</td></tr>)}</tbody>
              </table>
            </div>
          </Card>
        </div>
      ...
      {/* Approval lines tab */}
      {subTab==="approval" && (
        <Card title="Set Approval Lines">
          <div style={{color:C.g04,fontSize:12,marginBottom:16}}>Set who each employee reports to. Format: Name (Employee#) e.g. "Paul (166)"</div>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            <THead cols={["Name","Job Title","Reports To","Action"]}/>
            <tbody>{employees.map(emp=>(
              <tr key={emp["Employee #"]} style={{borderBottom:`1px solid ${C.g09}`}}>
                ...
                <td style={{padding:"10px 0"}}>
                  {editingId===emp["Employee #"]
                    ? <div style={{display:"flex",gap:6}}><Btn small onClick={()=>saveReporting(emp["Employee #"])}>Save</Btn><Btn small variant="ghost" onClick={()=>setEditingId(null)}>Cancel</Btn></div>
      ...
      {subTab==="invite" && (
        <div>
          <Card title="Invite Employee by Email" accent={C.yellow}>
            <div style={{color:C.g04,fontSize:12,marginBottom:16}}>Send an invitation email to a new employee to join the WFH tool.</div>
            <Field label="Email Address">
              <TextInput value={inviteEmail} onChange={e=>{setInviteEmail(e.target.value);setInviteMsg("")}} placeholder="newemployee@seoulrobotics.org"/>
            </Field>
            ...
            <div style={{color:C.g05,fontSize:11,marginTop:10}}>⚠ Real email sending requires a backend. This simulates the invite flow.</div>
          </Card>
          {invites.length > 0 && (
            <Card title="Sent Invites">
              <table style={{width:"100%",borderCollapse:"collapse"}}>
                <THead cols={["Email","Sent Date","Status"]}/>
                <tbody>{invites.map((inv,i)=><tr key={i} style={{borderBottom:`1px solid ${C.g09}`}}><td style={{color:C.white,fontSize:13,padding:"10px 16px 10px 0"}}>{inv.email}</td><td style={{color:C.g04,fontSize:13,padding:"10px 16px 10px 0"}}>{inv.sentAt}</td><td style={{padding:"10px 0"}}><Tag status="pending"/></td></tr>)}</tbody>
              </table>
            </Card>
          )}
        </div>
...
export default function App() {
  ...
  const newReq = (dates, reason) => {
    const newRecs = dates.map(d => ({
      id:`${user["Employee #"]}-${Date.now()}-${d.date}`,
      employeeId:user["Employee #"], team:user.Teams,
      date:d.date, half:d.half, status:"pending", reason
    }));
    setRecords(r=>[...r,...newRecs]); setModal(false);
  };

  if (!user) return <LoginScreen employees={employees} onLogin={login} invites={invites}/>;
  ...
  return (
    <div style={{minHeight:"100vh",background:C.black,fontFamily:"'Inter',sans-serif",color:C.white}}>
      ...
      <div style={{background:C.g09,borderBottom:`1px solid ${C.g08}`,padding:"0 28px",display:"flex"}}>
        {TABS.map(t=>(
          <button key={t.key} onClick={()=>setTab(t.key)} style={{background:"none",border:"none",borderBottom:tab===t.key?`2px solid ${C.yellow}`:"2px solid transparent",color:tab===t.key?C.white:C.g04,fontFamily:"inherit",fontSize:13,fontWeight:tab===t.key?600:400,padding:"14px 20px",cursor:"pointer",letterSpacing:"-0.01em"}}>{t.label}</button>
        ))}
      </div>
